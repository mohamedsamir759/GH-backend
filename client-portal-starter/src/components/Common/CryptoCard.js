...
function CryptoCard({ cryptoFooter, cryptoDetails, market, ...props }) {
  ...
  const borderStyle = buyButtonClass ? buyButtonClass : sellButtonClass ? sellButtonClass : "white";

  return (<>
    <Card 
      className="text-center crypto-card mb-4" 
      style={
        { 
          border: `0.25px solid ${borderStyle}` 
        }
      }
    >
      ...
      <div className={`btn-group crypto-footer w-100 ${props.hover ? "crypto-card-btn-hover" : "crypto-card-btn"}`} role="group">
        ...
        {cryptoFooter ? cryptoFooter : (
          <>
            <Button type="button" className="btn btn-light btn-sm waves-effect waves-light py-2 quick-buy-button" onClick={() => {
...
const mapStateToProps = (state) => {
  return {
    ...
    highKlinesLoading: state.klines.loading,
  };
};
