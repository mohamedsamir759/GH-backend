...
function Transactions(props) {
  ...
  return (
    <React.Fragment>
      ...
      />
      { [...props.deposits, ...props.withdrawals].length > 0 &&
        <Button 
          type="button" 
          onClick = {() => {
            if (props.depositsTotalDocs > props.deposits.length || props.withdrawalsTotalDocs > props.withdrawals.length){
              setLimit(limit + 5);
            }
          }}
...
const mapStateToProps = (state)=>(
  {
    deposits: state.historyReducer.deposits || [],
    withdrawals :state.historyReducer.withdrawals || [],
    withdrawalsTotalDocs : state.historyReducer.withdrawalsTotalDocs,
    depositsTotalDocs : state.historyReducer.depositsTotalDocs,
    depositLoading: state.historyReducer.depositLoading,
    withdrawalLoading: state.historyReducer.withdrawalLoading,
  }
);
