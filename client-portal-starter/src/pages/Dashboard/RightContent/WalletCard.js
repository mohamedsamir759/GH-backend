  ...
  return (
    <CardWrapper className='mb-5'>
      ...
      <div className="btn-group w-100 d-flex align-items-center justify-content-between" role="group" style={{
        ...
        <Button type="button" className='btn btn-danger' onClick={() => { dispatch(toggleCurrentModal("fiatWithdraw")) }}>{props.t("Withdraw")}</Button>
        <Button type="button" className='btn btn-success' onClick={() => { dispatch(toggleCurrentModal("fiatDeposit")) }}>{props.t("Deposit")}</Button>
      </div>
