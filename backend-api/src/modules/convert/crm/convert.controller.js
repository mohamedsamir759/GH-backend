...
class ConvertController {
  async createConvert(req, res, next) {
    try {
      ...
      const response = req.customer && req.customer.category === CONSTANTS.CUSTOMER_TYPES.LIVE
  ...
  async getRecords(req, res, next) {
    try {
      const rec = req.customer && req.customer.category === CONSTANTS.CUSTOMER_TYPES.LIVE
        ? await service.find()
        : await dservice.find();
