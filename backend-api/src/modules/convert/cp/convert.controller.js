...
class ConvertController {
  async createConvert(req, res, next) {
    try {
      ...
      const response = req.user && req.user.category === CONSTANTS.CUSTOMER_TYPES.LIVE
  ...
  async getPaginate(req, res, next) {
    try {
      ...
      const rec = req.user && req.user.category === CONSTANTS.CUSTOMER_TYPES.LIVE
        ? await service.newFindWithPagination(query, populate)
        : await dservice.newFindWithPagination(query, populate);
      return ApiResponse(res, true, ResponseMessages.RECORD_FETCH_SUCCESS, rec);
    } catch (error) {
      return next(error);
    }
