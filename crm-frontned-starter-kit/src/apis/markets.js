...
export const searchMarketFromExchangeAPI = async ({ payload }) => {
  const data = await axiosHelper.get(`/markets/search?${qs.stringify(payload)}`, { crypto: true });
  return data;
};

export const updateMarketConfiguration = async (payload) => {
  const data = await axiosHelper.patch("/markets/config", payload);
  return data;
};
