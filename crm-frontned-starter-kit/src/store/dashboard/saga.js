...
import {
  ...
  fetchOperationStagesStatsFailed,
  fetchOperationStagesStatsStart,
  fetchOperationStagesStatsSuccess
} from "./actions";
...
function* fetchOperationStages(params) {
  try {
    const data = yield call(dashboardApi.getOperaionStages, params);
    yield put(fetchOperationStagesStatsSuccess(data));
  }
  catch (error) {
    yield put(fetchOperationStagesStatsFailed(error));
    yield put(showErrorNotification(error.message || "Error Fetching Operation stats"));
  }
}
...
function* authSaga() {
  ...
  yield takeEvery(FETCH_OPERATION_STAGES_STATS_START, fetchOperationStages);
}
