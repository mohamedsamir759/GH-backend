...
import {
  ...
  addNewIbSuccess,
  fetchClientStatisticsSuccess,
  fetchClientStatisticsFail,
  fetchClientStatusticsClear,
} from "./actions";
import { 
  ADD_NEW_CLIENT, 
  FETCH_CLIENTS_START,
...
function * fetchClientStatistics(params){
  try {
    const data = yield call(clientApi.getClientStatisticsById, params);
    yield put(fetchClientStatisticsSuccess(data));
    yield delay(2000);
    yield put(fetchClientStatusticsClear());
  } catch (error){
    yield put(fetchClientStatisticsFail({ error: error.message }));
  }
}
...
function* clientSaga() {
  ...
  yield takeEvery(FETCH_CLIENT_STATISTICS_REQUESTED, fetchClientStatistics);
