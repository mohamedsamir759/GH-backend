...
import { searchMarketFromExchangeAPI } from "apis/markets";

function MarketAdd(props) {
  const [addModal, setAddUserModal] = useState(false);
  const [baseAsset, setBaseAsset] = useState("");
  const [quoteAsset, setQuoteAsset] = useState("");
  const [isSpotEnabled, setIsSpotEnabled] = useState(false);
  const [isMarginEnabled, setIsMarginEnabled] = useState(false);
  const [isFuturesEnabled, setIsFuturesEnabled] = useState(false);
