...
function OrderList(props) {
  ...
  const getData = async () => {
    try {
      console.log("dasdasdadadas1111111111111111");
      const payload = {
        page : 1,
        limit : 20,
        customerId : clientId
      ...
      const x = await getOrders(payload);
      console.log("dasdasdadadas", x);
      
    } catch (error) {
      console.log("dasdasdadadas", error);
    }
  };

  useEffect(()=>{
    getData();
    console.log("eqweqwqeweqeqwdocsdocsdocs", docs);
  }, [docs]);
  return (
    <React.Fragment>
